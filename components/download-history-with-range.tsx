...
import {
  getDriverTransactionsByRange,
  getDriverRidesByRange,
  getParentTransactionsByRange,
  getParentRidesByRange,
} from "@/lib/api"
...
interface DownloadHistoryWithRangeProps {
...
export function DownloadHistoryWithRange({ type, title, description, userType }: DownloadHistoryWithRangeProps) {
  ...
  const fetchRecordCount = async () => {
    ...
    try {
      ...
      if (type === "transactions") {
        if (userType === "driver") {
          data = await getDriverTransactionsByRange(timeRange, showCustomDateRange ? customDateRange : undefined)
        } else {
          data = await getParentTransactionsByRange(timeRange, showCustomDateRange ? customDateRange : undefined)
        }
      } else {
        if (userType === "driver") {
          data = await getDriverRidesByRange(timeRange, showCustomDateRange ? customDateRange : undefined)
        } else {
          data = await getParentRidesByRange(timeRange, showCustomDateRange ? customDateRange : undefined)
        }
      }
...
export default DownloadHistoryWithRange
