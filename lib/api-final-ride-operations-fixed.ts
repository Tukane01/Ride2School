...
// Enhanced complete ride function with proper timestamp handling
export const completeRideFinal = async (rideId: string) => {
  ...
  try {
    ...
    // Call the database function to complete the ride with proper timestamp
    const { data, error } = await supabase.rpc("move_ride_to_completed", {
      p_ride_id: rideId,
      p_actual_pickup_time: null,
      p_actual_dropoff_time: new Date().toISOString(), // Ensure proper timestamp format
...
// Enhanced cancel ride function with proper error handling
