-- Fix messages table - add missing archived_at column
ALTER TABLE messages ADD COLUMN IF NOT EXISTS archived_at TIMESTAMPTZ;

-- Drop existing conflicting functions
DROP FUNCTION IF EXISTS move_ride_to_completed(UUID, TIMESTAMPTZ, TIMESTAMPTZ, DECIMAL, INTEGER);
DROP FUNCTION IF EXISTS move_ride_to_completed(UUID, TIMESTAMP, TIMESTAMP, DECIMAL, INTEGER);

-- Create the definitive ride completion function with proper timestamp handling
...
BEGIN
    ...
    BEGIN
        ...
        VALUES 
            (v_ride_record.parent_id, 'Ride Completed', 'Your child''s ride has been completed successfully.', 'ride_completed', v_ride_record.id, NOW()),
            (v_ride_record.driver_id, 'Ride Completed', 'You have successfully completed the ride and earned R' || v_ride_record.fare::text || '.', 'ride_completed', v_ride_record.id, NOW());
...
-- Drop existing conflicting cancellation functions
DROP FUNCTION IF EXISTS move_ride_back_to_requests(UUID, TEXT);

-- Create the definitive driver cancellation function
...
BEGIN
    ...
    BEGIN
        ...
        INSERT INTO notifications (user_id, title, content, type, ride_id, created_at)
        VALUES 
            (v_ride_record.parent_id, 'Ride Cancelled', 'Your ride has been cancelled by the driver. The ride is now available for other drivers to accept.', 'ride_cancelled', v_ride_record.id, NOW()),
            (v_ride_record.driver_id, 'Ride Cancelled', 'You have cancelled the ride. A penalty of R' || v_penalty_amount::text || ' has been applied to your account.', 'ride_cancelled', v_ride_record.id, NOW());
...
-- Drop existing conflicting parent cancellation functions
DROP FUNCTION IF EXISTS move_ride_to_cancelled(UUID, UUID, TEXT);

-- Create the definitive parent cancellation function
...
BEGIN
    ...
    BEGIN
        ...
        INSERT INTO notifications (user_id, title, content, type, ride_id, created_at)
        VALUES 
            (v_ride_record.parent_id, 'Ride Cancelled', 'You have cancelled the ride. A penalty of R' || v_penalty_amount::text || ' has been applied to your account.', 'ride_cancelled', v_ride_record.id, NOW()),
            (v_ride_record.driver_id, 'Ride Cancelled', 'The ride has been cancelled by the parent.', 'ride_cancelled', v_ride_record.id, NOW());
